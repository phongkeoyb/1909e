<?php

session_start();


require_once ('database.php');
$database = new Database();
if(isset($_POST) && !empty($_POST)){
    if(isset($_POST['action'])){
        switch ($_POST['action']){
            case 'add':
                if (isset($_POST['quantity']) && isset($_POST['product_id'])){
                    $sql = "SELECT * FROM products WHERE id =".(int)$_POST['product_id'];
                    $product = $database->runQuery($sql);
                    $product = current($product);
                    $product_id = $product["id"];
                    $cart_item = array();
                    //khi gio hang ton tai va gio hang khong rong
                    if (isset($_SESSION["cart_item"]) && !empty($_SESSION["cart_item"])){
                        //check xem san pham da add co ton tai trong gio hang hay chua
                        //neu ton tai thi so luong mua moi + so luong mua truoc do
                        if (isset($_SESSION["cart_item"][$product_id])){
                            //san pham da ton tai tron gio hang
                            $exist_cart_item = $_SESSION["cart_item"][$product_id];
                            $exist_quantity = $exist_cart_item['quantity'];

                            $cart_item['id'] = $product["id"];
                            $cart_item['product_name'] = $product["product_name"];
                            $cart_item['product_image'] = $product["product_image"];
                            $cart_item['price'] = $product["price"];
                            $cart_item['quantity'] = $_POST["quantity"] + $exist_quantity;
                            $_SESSION["cart_item"][$product_id] = $cart_item;
                        }else{
                            //san pham chua ton tai trong gio hang thi se them moi
                            $cart_item['id'] = $product["id"];
                            $cart_item['product_name'] = $product["product_name"];
                            $cart_item['product_image'] = $product["product_image"];
                            $cart_item['price'] = $product["price"];
                            $cart_item['quantity'] = $_POST["quantity"];
                            $_SESSION["cart_item"][$product_id] = $cart_item;
                        }
                    } else{
                        $_SESSION["cart_item"] = array();
                        $cart_item['id'] = $product["id"];
                        $cart_item['product_name'] = $product["product_name"];
                        $cart_item['product_image'] = $product["product_image"];
                        $cart_item['price'] = $product["price"];
                        $cart_item['quantity'] = $_POST["quantity"];
                        $_SESSION["cart_item"][$product_id] = $cart_item;
                    }
                }
                break;
            case 'remove':
                echo "Xóa";
                echo "<br> POST";
                echo "<pre>";
                print_r($_POST);
                echo "</pre>";
                if(isset($_POST['product_id'])){
                    $product_id = $_POST['product_id'];
                    if(isset($_SESSION["cart_item"][$product_id])){
                        unset($_SESSION["cart_item"][$product_id]);
                    }
                }
                break;
            default:
                echo "Action không tồn tại!!!";
                die;
        }
    }

}
header('Location: http://localhost/1909e/BT_Simple_shop/index.php?');

die();